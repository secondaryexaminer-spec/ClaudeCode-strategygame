...
`onBoard` 的每条分支、摄像机、键盘各验一条。局面用 `__frontierDebug.placeUnit`
直接摆出来 —— 随机开局不保证出现「运兵船旁边有陆军」这类组合。
...
| 拖拽 | 右键平移、左键不平移 | `beginPan` 去掉按键判断 → 红 |
...
- **绑定只验"挂上了"，不验"挂对了"**。点了会发生什么、参数传得对不对、
  toast 文案合不合适，一概没测。面板上那些按钮（生产、升级、变卖、工程师建造）
  的点击处理**没有任何覆盖** —— 交互链走的是棋盘点击，不是面板按钮。
- **大厅的 change 事件重渲染没覆盖**：`renderLobbyPreview` 只在初始化时跑到一次，
  换地图 / 换尺寸之后的重画没测。选项内容对不对也不验。
- **存档链路完全没测**：保存、读档、导入导出一条都没跑过。
